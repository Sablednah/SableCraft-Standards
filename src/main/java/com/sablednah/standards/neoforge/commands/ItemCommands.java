...
public final class ItemCommands {
    ...
    private static int give(CommandContext<CommandSourceStack> ctx, int count)
        ...
        // 26.x dropped the 'allow oversized stacks' boolean that 1.21.11 takes here — see
        // CROSS-VERSION.md, it is the divergence this command carries. Behaviour is the same
        // either way, because the loop below chunks by the real maximum.
        ItemStack probe = input.createItemStack(1);
        ...
        while (given < wanted) {
            ...
            ItemStack stack = input.createItemStack(size);
